...
INSERT INTO DangNhap
    (TaiKhoan, MatKhau, Quyen, NhoMatKhau)
VALUES
...
INSERT INTO NhanVien
    (MaNV, TenNV, GioiTinh, NgaySinh, DiaChi, SDT, CMND, Luong)
VALUES
...
INSERT INTO Phong
    (MaPhong, TenPhong, DienTich, TienNghi, Gia, TrangThai)
VALUES
...
BEGIN
    UPDATE DichVuDaThue
        SET TongTien = SoGio * (SELECT GiaTrenGio
    FROM DichVu
    ...
    UPDATE ThuePhong
		SET TongTien = ((SELECT Gia
    FROM Phong, ThuePhong
    WHERE Phong.MaPhong = ThuePhong.MaPhong AND ThuePhong.MaThue = @MaThue)
         * (SELECT DATEDIFF(DAY, NgayBD, NgayKT)
    FROM ThuePhong
    WHERE MaThue = @MaThue)) 
         + (SELECT SUM(TongTien)
    FROM DichVuDaThue
...
SELECT *
FROM DoDungTrongPhong

INSERT INTO DoDungTrongPhong
    (MaPhong, TenDo, SoLuong)
VALUES
    ('PH12', 'Tivi', 66);
...
INSERT INTO ThuePhong
    (MaThue, MaPhong, MaKH, NgayBD, NgayKT, TongTien, TrangThai)
VALUES
...
BEGIN
    ...
    UPDATE ThuePhong 
		SET TongTien = @TongTien * (SELECT DATEDIFF(DAY, NgayBD, NgayKT)
    FROM INSERTED)
